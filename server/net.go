...
func Init( ){
	...
	IpPoolSize =  1<< 14
	usage = make([]byte, IpPoolSize)//1 means can be use, and 0 means can not be use
...
func Update(){
	var count int32
	var bytes [4]byte
	bytes[0] =  MasterConfig.IpPool.Net[12] 
	bytes[1] =  MasterConfig.IpPool.Net[13] 
	var i, j byte
	for i=0;i<=255;i++ {
		for j=0;j<=255;j++{
			bytes[2]= i
...
func Index2Ip( index int32)(net.IP, error){
	...
	var err error
	var ip [4]byte
	ip[0] = MasterConfig.IpPool.Net[12]
	ip[1] = MasterConfig.IpPool.Net[13]
	ip[2] = byte(index/256)
	ip[3] = byte(index%256 )
